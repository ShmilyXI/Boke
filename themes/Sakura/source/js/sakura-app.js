...
mashiro_global.lib = new (function () {
  ...
  this.hasClass = function (ele, className) {
    ...
    if (el.classList) {
      ...
      var e = new RegExp("(^| )" + className + "( |$)", "gi").test(
        el.className
      );
    }
    return e;
...
function removeCookie(name) {
  document.cookie =
    name + mashiro_option.cookie_version_control + "=; Max-Age=-99999999;";
}
    ...
    case 1:
      if (ele.src.includes("https://cn.gravatar.com/avatar")) {
        ele.src = ele.src.replace(
          ...
          "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/other/gravatar.jpg"
        );
      } else {
        ele.src =
          "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/other/default_avatar.jpg";
      }
      break;
    case 2:
      ele.src =
        "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/other/default_gavatar.jpg";
      break;
    case 3:
      ele.src =
...
mashiro_global.post_list_show_animation = new (function () {
  this.ini = function (ajax) {
    $("article.post-list-thumb").each(function (i) {
      ...
      var article_height = $("article.post-list-thumb")
        .eq(i)
        .offset().top;
      if ($(window).height() + $(window).scrollTop() >= article_height) {
        $("article.post-list-thumb")
          .eq(i)
          .addClass("post-list-show");
      }
    ...
    for (var i = 0; i < this.files.length; i++) {
      ...
      $.ajax({
        ...
        success: function (res) {
          ...
          var get_the_url = res.data.url.replace(
            "https://i.loli.net/",
            "https://static.shino.cc/user-upload/"
          );
          $("#upload-img-show").append(
            '<img class="lazyload upload-image-preview" src="https://cdn.jsdelivr.net/gh/moezx/cdn@3.0.2/img/svg/loader/trans.ajax-spinner-preloader.svg" data-src="' +
            get_the_url +
            '" onclick="window.open(\'' +
            get_the_url +
          ...
          grin(
            res.data.url.replace("https://i.loli.net/", "{UPLOAD}"),
            (type = "Img")
          );
        },
        error: function () {
...
function checkBgImgCookie() {
  ...
  if (bgurl != "") {
    if (
      bgurl ==
      "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/themebg/sakura.png" ||
      bgurl ==
      "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/themebg/plaid.jpg" ||
      bgurl ==
      "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/themebg/star.png" ||
      bgurl ==
      "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/themebg/point.png" ||
      bgurl ==
      "https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/themebg/little-monster.png"
    ) {
...
$(document).ready(function () {
  ...
  $(".skin-menu #dark-bg").click(function () {
    ...
    $("body").css(
      "background-image",
      "url(https://cdn.jsdelivr.net/gh/honjun/cdn@1.6/img/other/starry_sky.png)"
    );
...
function nextBG() {
  ...
  $(".centerbg").css(
    "background-image",
    'url("' + bg[Math.abs(bgindex % bg.length)] + '")'
  );
}
...
function add_copyright() {
  document.body.addEventListener("copy", function (e) {
    if (
      !mashiro_global.is_user_logged_in &&
...
function inlojv_js_getqqinfo() {
  ...
  if (
    !getCookie("user_qq") &&
    !getCookie("user_qq_email") &&
    !getCookie("user_author")
  ) {
    $("input#qq,input#author,input#email,input#url").val("");
  }
  if (
    getCookie("user_avatar") &&
    getCookie("user_qq") &&
    getCookie("user_qq_email")
  ) {
  ...
  $("input#author").on("blur", function () {
    ...
    $.ajax({
      ...
      error: function () {
        ...
        $("div.comment-user-avatar img").attr(
          "src",
          get_gravatar($("input#email").val(), 80)
        );
